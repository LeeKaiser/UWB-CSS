...
//---------------------------------------------------------------------------
// Insert
// accept nodeData object then inserts it to the appropriate section in 
// the binary tree
...
// looks through the binary tree to find if a NodeData object is in the 
// binary tree. returns true if found, false otherwise. 
// sets accept as the data if found.
bool BinTree::retrieve(const NodeData&find , NodeData*&accept ) const
{
...
bool BinTree::retrieveHelper(const NodeData& find, NodeData*& accept, 
   const Node* current) const
{
   ...
   if (*current->data == find) // found the data
   {
...
bool BinTree::remove(const NodeData &find, NodeData*&accept)
{
   return removeHelper(find, accept, root);
}
//---------------------------------------------------------------------------
// removeHelper
bool BinTree::removeHelper(const NodeData&find , NodeData*&accept, 
   Node*&current)
{
...
void BinTree::deleteCurrent(Node*& current) {
...
//---------------------------------------------------------------------------
// findPredAndDelete
// finds the predecessor of current in deleteCurrent then has its NodeData
// replace the data of current.
//assumes it puts in left of current in the helper 
NodeData* BinTree::findPredAndDelete(Node*& current)
{
   //if there is nothing on right
   if (current->right == nullptr) {
      //cout << *current->data << endl;
      NodeData* item = current->data; //returns this
      current->data = nullptr;
      Node* junk = current; 
...
// returns false
bool BinTree::getSibling(const NodeData& find, NodeData& accept) const
{
...
bool BinTree::getSiblingHelper(const NodeData& find, NodeData& accept, 
   Node* current) const
{
   if (current == nullptr) // reached nullptr because couldn't find element
   {
...
bool BinTree::getParent(const NodeData&find, NodeData&accept) const
{
...
// the user is seeking (find)
bool BinTree::getParentHelper(const NodeData& find, NodeData& accept, 
   Node* current) const
{
...
{
   //stop when reaching edge of bintree
   if (current != nullptr)
   {
      //put left of tree
...
//---------------------------------------------------------------------------
// arrayToBSTreeHelper
void BinTree::arrayToBSTreeHelper(Node*& current, NodeData* arr[], 
   int min, int max)
{
   //get mid
   int mid = (max + min) / 2;
...
//---------------------------------------------------------------------------
// inOrderHelper
// recursive function that is used to help with putting the bintree 
...
BinTree::Node* BinTree::copyRecursive (Node* other)
{
   if (other != nullptr)
   {
      Node* ptr = new Node; // copy other's data
      NodeData* nd = new NodeData(*other->data);
      ptr->data = nd;
      ptr->left = ptr->right = nullptr;
...
bool BinTree::findFalse(const Node* current, const Node* other) const
{
...
ostream &operator<<(ostream &os, const BinTree &obj)
{
...
//---------------------------------------------------------------------------
// = operator overload
// if this and other is not the same bintree, copy other into this
